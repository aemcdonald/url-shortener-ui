...
describe('UrlContainer', () => {
  it('Should render the title & the long & short urls', () => {
    ...
    const { getByTestId, getByText } = render(<UrlContainer urls={mockUrls} deleteURL={mockDelete}/>)
  ...
  it('fire a fn when the delete button clicked', () => {
    const mockDelete = jest.fn()
    const mockUrls = [
      {
        id: 8,
    ...
    const { getByTestId, getByText, queryAllByText } = render(<UrlContainer urls={mockUrls} deleteURL={mockDelete}/>)

    const deleteBtns = queryAllByText('Delete')
